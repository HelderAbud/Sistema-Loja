...
|-------|-----------|----------------|
| **Use case** | Orquestração transacional: idempotência HTTP, vários agregados, audit | `com.lojapp.application.usecases.*` (`contract` e `idempotency` ficam em `application`) |
...
- Repositório **público** no GitHub; correções **graduais** conforme `docs/lojapp/plano-consolidado-melhorias-2026-05-24.md`.
- Ordem acordada: **Fase 0** → **A1 → A2 → A3 → A4** → **B1 → B2 → B3 → B4** → Fases C–F.
- Gate publicação portfolio: Fase A + B + P0 do `docs/CHECKLIST_FINAL.md`.
