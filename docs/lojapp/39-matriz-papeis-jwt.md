...
No fechamento de caixa, `managerApproval` / `managerApprovalRequired` são **auto-declaração** (o próprio ator confirma que reviu a diferença acima da tolerância) e entram no log de auditoria. Não há PIN, segundo token nem checagem de um gerente na mesma tenancy — isso só faria sentido com loja ≠ login (fora do MVP). Ver `docs/CONTEXT.md`.
