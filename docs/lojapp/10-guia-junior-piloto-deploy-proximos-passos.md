...
| 2 | **Subir infra (Docker)** | PostgreSQL 16 a escutar (porta host **5433**). |
| 3 | **Backend (API)** | `mvn spring-boot:run` na **raiz** - sobe Spring Boot, aplica **Flyway**, expõe REST em **http://localhost:8081**. |
| 4 | **Frontend (SPA)** | `cd frontend` → `npm install` → `npm run dev` — Vite em **http://localhost:5173** com **proxy** `/api` → API. |
...
- [ ] Sabes que **porta 8081** = API e **porta 5173** = Vite (ver `frontend/vite.config.ts`).
...
| **Documentação viva da API** | Swagger UI - **http://localhost:8081/swagger-ui.html** (com API em perfil default) |
...
- Porta **5433** livre na máquina (ou alteras o mapeamento no compose com consciência de que o `application.yml` tem de bater certo).
...
- **Swagger:** http://localhost:8081/swagger-ui.html (porta por defeito; se mudaste `server.port`, ajusta o URL).
...
- O servidor de desenvolvimento Vite sobe em **http://localhost:5173** (definido em `vite.config.ts`).  
- Os pedidos a **`/api/...`** são **encaminhados por proxy** para **http://localhost:8081** - desde que a API esteja a correr.
...
|---------|----------------|
| Erro 502 / "Falha no proxy" no browser | API **não** está na porta 8081 ou não arrancou. |
...
- [ ] `npm run dev` indica URL local (ex.: `:5173`)  
...
| 3 | Frontend em branco / 502 em `/api` | API na **8081**? `mvn spring-boot:run` ativo? |
...
- [ ] Browser: **http://localhost:8081/swagger-ui.html** abre (porta por defeito; ajuste se mudou `server.port`).
...
- Saúde: `curl -s http://localhost:8081/actuator/health` (ajusta porta se alterares o mapeamento no compose).
- OpenAPI desligado: `curl -s -o NUL -w "%{http_code}" http://localhost:8081/v3/api-docs` - esperado **404** (ou não **200**), pois `springdoc.api-docs.enabled: false` em `application-prod.yml`.
- Login: `POST http://localhost:8081/api/v1/auth/login` com JSON email/password; deve devolver JWT.
